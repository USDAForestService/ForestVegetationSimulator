...
C
	...
	LOGICAL LSPPOK(MAXSP)
      LOGICAL L_IsICH, L_IsIDF, L_IsSBS, L_IsSBPS
...
C     SITE SERIES COEFFICIENTS
     ...
     >    (/ 8, (0, I=1,14) /),
     >    (/       'IDF/all',          'SBS/all',         'SBPS/all',
     >        ('', I=1,27)/),
     >    0.0
     >    ),

     >  SS_STR (  ! ICH BL,BG 15m 1-9 (20)
     >    (/ 9, 4, (0, I=1,13)  /),
     ...
     >  SS_STR (  ! ICH+IDF EP,OH,AT,AC (zonal, all hardwoords) (25)
     >    (/11, 12, 13, 15, (0, I=1,11)  /),
     >    (/       'IDF/all',         'ICH/all',             'SBS/all',
     >            'SBPS/all',   ('', I=1,26) /),
     >    0.0
     >    ) /
...
C     Identify BEC for faster checking
      L_IsICH = .FALSE.
      L_IsIDF = .FALSE.
      L_IsSBS = .FALSE.
      L_IsSBPS = .FALSE.
      ...
      ELSEIF (INDEX(BEC%Zone,'SBS') .GT. 0) THEN
         L_IsSBS = .TRUE. 
      ELSEIF (INDEX(BEC%Zone,'SBPS') .GT. 0) THEN
         L_IsSBPS = .TRUE.
      ENDIF
...
C         PL MODEL IN IDF MUST HAVE AT CCF>=100
     ...
     >          RELDN2 = MAX(125.0, RELDEN)
            CASE (11,15)
              IF (L_IsICH .OR. L_IsIDF)
     >          RELDN2 = MAX(100.0, RELDEN)
            END SELECT
          
          ! ICH/IDF DG models use units of CCF/100
          IF (L_IsICH .OR. L_IsIDF) RELDN2 = RELDN2 * 0.01
...
C           EP/OH MODEL MUST HAVE CROWN<=0.80
     ...
     >               + DGCR1(ISPC)   * CR
            ...
            ELSEIF (L_IsSBS .OR. L_IsSBPS) THEN
               
               ! some BEC/spp use LN() transform; others untransformed
               IF (ISPC .EQ. 7 .AND. L_IsSBS) THEN
                  D2 = MAX(1.0,D)
               ELSEIF (ISPC .EQ. 9) THEN
                  D2 = MAX(1.0,D)             
               ELSE 
                  D2 = LOG(MAX(1.0,D))
               ENDIF
