...
C     ARE MSE AND ADDITIVE TO THE PREDICTED HEIGHT GROWTH.
     ...
     >  ST_STR (
     >    (/ 4, 9, 5, 8, 6,
     >       (0,I=1,10) /),! ICH - TOLERANT (BG,BL,HW,SE,CW)  (15)
     >    (/'ICH/all', ('',I=1,29)/),           ! BEC list
     >      -0.0878875,            ! constant
     >       0.0,                  ! cos-aspect*slope
     >       0.0,                  ! sin-aspect*slope
     ...
     >       0.43),                ! SE=0.43, n=342
     >
!     In the 2005 FSP report, Zumrawi et al. fitted two small tree height models;
!     one for smaller small-trees (<1.37) and one for larger small trees. The
!     model can only use one, so the larger small-tree model is used. The 
!     smaller tree models are included for completeness. - DR.

!     >  ST_STR (
!     >    (/3,(0,I=1,14)/), ! Fd - SBS     <1.37m height model
...
!     >       16.33),                ! SE=16.33, n=55
     ...
     >
!     >  ST_STR (
!     >    (/8,(0,I=1,14)/), ! Sx - SBS   <1.37m height model
!     >    (/'SBPSmc/01', 'SBPSmk/01',  'SBPSdc/01',
!     >      'SBPSxc/01', 'SBSdw1/01',  'SBSdw2/01',
!     >      'SBSmc1/01', 'SBSmc2/01',   'SBSmh/01', 
!     >       'SBSmm/01',  'SBSmw/01',  'SBSwk1/01',
...
!     >       25.59),                ! SE=25.59, n=77
     ...
     >
     >  ST_STR (
     >    (/11,12,15, (0,I=1,12)/), ! SBS - At/EP              (20)
     >    (/'SBPSmc/01', 'SBPSmk/01',  'SBPSdc/01',
     >      'SBPSxc/01', 'SBSdw1/01',  'SBSdw2/01',
     >      'SBSmc1/01', 'SBSmc2/01',   'SBSmh/01', 
     >       'SBSmm/01',  'SBSmw/01',  'SBSwk1/01',
     ...
     >       0.0,                  ! bal/100
     >       1.00,                 ! Multiplier !SBS only
     >      -19.642,               ! HT/CCF     !SBS only 
     >      -238.1,                ! CCFS/CCFL  !SBS only
     >       78.89)/               ! SE=78.89, n=21
     >
!     >  ST_STR ( ! OLD MODEL BY TERRY LEE
!     >    (/12, (0,I=1,14)/),       ! SBS - At              (20)
!     >    (/'SBPSmc/01', 'SBPSmk/01',  'SBPSdc/01',
!     >      'SBPSxc/01', 'SBSdw1/01',  'SBSdw2/01',
!     >      'SBSmc1/01', 'SBSmc2/01',   'SBSmh/01', 
!     >       'SBSmm/01',  'SBSmw/01',  'SBSwk1/01',
...
C     BUT NOT INCLUDING THE CURRENT RECORD. SORTING IS BY DBH.
      ...
      IF (.NOT. LV2ATV .AND. (L_IsSBS .OR. L_IsSBPS)) THEN
        SBSCCF = 0.0 
        DO I = 1,ITRN
...
C           FOR "HISTORICAL PURPOSES" ONLY.
     ...
     >            (STG%CCFSL(ISPC) * MIN(CCFS,0.5)/MAX(CCFL,36.)))))
...
C  USED IN NI; HTGR CANNOT BE NEGATIVE!
   ...
   18     ZZRAN=0.0
          ...
          ELSE
            HTGR = MAX(0.0, HTGR1 + ZZRAN*STG%SD(ISPC)*CMtoFt)
          ENDIF
