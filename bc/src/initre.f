...
C----------
      ...
      IF (ICL1.EQ.0) CALL ERRGRO(.FALSE.,2)
      IREAD=ICL1
      ICL1=0
      GOTO 10
   20 CONTINUE
...
C----------
        IF (.NOT.LNOTBK(3)) ARRAY(3)=DBHMIN(IS)
        IF (LNOTBK(4).AND.(ITOPD8.GT.0.)) ARRAY(4)=TOPD(IS)
        IF (.NOT.LNOTBK(5)) ARRAY(5)=STMP(IS)
        IF (.NOT.LNOTBK(6)) ARRAY(6)=FRMCLS(IS)
        IF (.NOT.LNOTBK(7)) ARRAY(7)=METHC(IS)
        CALL OPNEW(KODE,IDT,217,6,ARRAY(2))
...
C
 4600 CONTINUE
      ...
      ARRAY(5) = ARRAY(5)*CMtoFT
C
C  SET ITOPD8=1 1 TO DISSALLOW MODIFICATION OF TOP MERCH. SPEC.
C  IN R8 VARIANTS
C
      ITOPD8=0
      IF(VVER(:2).EQ.'SN')ITOPD8=1
...
C----------
        ...
        IF (LNOTBK(4).AND.(ITOPD8.LE.0)) BFTOPD(IS)=ARRAY(4)
        IF (LNOTBK(5)) BFSTMP(IS)=ARRAY(5)
        IF (LNOTBK(6)) FRMCLS(IS)=ARRAY(6)
        IF (LNOTBK(7)) METHB(IS) =ARRAY(7)
        ILEN=3
...
C----------
      IF(IS .LT. 0) THEN
        ...
        IF (LNOTBK(4).AND.(ITOPD8.GT.0)) ARRAY(4)=BFTOPD(IFSP)
...
C----------
      ELSEIF(IS .EQ. 0)THEN
        ...
        IF (LNOTBK(4).AND.(ITOPD8.GT.0)) ARRAY(4)=BFTOPD(1)
