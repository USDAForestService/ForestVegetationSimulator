...
C
C SETTING FOR SPECIES 18 AND 16 IS FROM THE ORGANON CODE
C----------
      IF(DEBUG)WRITE(JOSTND,*)' IN SITSET BEFORE ORG SITE EQNS SP 18 &',
     &' 7 SITE INDEX = ',SITEAR(18),SITEAR(7)
      IF(SITEAR(7) .LE. 0.0 .AND. SITEAR(18) .GT. 0.0)THEN
...
C
C  IF STAND AGE FROM STDINFO KEYWORD IS 0 SET MANAGEMENT FLAG TO 
C  MANAGED AND EVEN/UNEVEN FLAG TO EVEN-AGED STAND
C----------
      IF(IAGE .EQ. 0) THEN
        INDS(4)=1
        MANAGD=1
