...
C       CROSSWALK FOR RESERVATION PSUEDO CODES & LOCATION CODE
        CASE (8117)
          WRITE(JOSTND,60)
   60     FORMAT(T12,'UMATILLA RESERVATION (8117) BEING MAPPED ',
     &    'TO UMATILLA NF (614) FOR FURTHER PROCESSING.')
          IFOR = 3
...
C     FOREST MAPPING CORRECTION
      SELECT CASE (IFOR)
        CASE (5)
          WRITE(JOSTND,21)
   21     FORMAT(T12,'WHITMAN NF (619) BEING MAPPED TO WALLOWA-',
     &    'WHITMAN (616) FOR FURTHER PROCESSING.')
          IFOR = 4
      END SELECT
