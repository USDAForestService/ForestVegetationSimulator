      ...
      IMPLICIT NONE
C----------
C  **CROWN--EC    DATE OF LAST REVISION:  03/31/11
C----------
...
COMMONS
     ...
     & CRNMLT(11),DLOW(11),DHI(11),
     & PRM(5)
      INTEGER ISORT(MAXTRE),MYACTS(1),ICFLG(11)
      ...
      DATA MYACTS/81/
C
C  SPECIES ORDER 1=WP, 2=WL, 3=DF, 4=WF, 5=MH, 6=GF, 7=LP, 8=ES,
C                9=NF, 10=PP, 11=OT
C-----------
...
C
C SPECIES ORDER
C  1=WP,  2=WL,  3=DF,  4=SF,  5=RC,  6=GF,  7=LP,
C  8=ES,  9=AF, 10=PP, 11=OT
C
...
C
      DATA WEIBB0/  0.08106,  0.00603, -0.28295, -0.09734, -0.01129,
     1   -0.09734, -0.00047, -0.15678,  0.08247,  0.08106, -0.01129/
C
C LOAD WEIBULL 'B' PARAMETER EQUATION SLOPE COEFFICIENT
C
      DATA WEIBB1/  1.10253,  1.12276,  1.18232,  1.14675,  1.11665,
     1    1.14675,  1.13172,  1.14894,  1.10804,  1.10253,  1.11665/
C
C LOAD WEIBULL 'C' PARAMETER EQUATION CONSTANT COEFFICIENT
C
      DATA WEIBC0/  1.04477,  2.73400,  3.03400,  2.71600,  3.35500,
     1    2.71600,  2.22700,  3.05300,  1.45931,  1.04477,  3.35500/
C
C LOAD WEIBULL 'C' PARAMETER EQUATION SLOPE COEFFICIENT
C
      DATA WEIBC1/  0.42828,  0.00000,  0.00000,  0.00000,  0.00000,
     1    0.00000,  0.00000,  0.00000,  0.25495,  0.42828,  0.00000/
C
C LOAD CR=F(SDI) EQUATION CONSTANT COEFFICIENT
C
      DATA C0/      5.23986,  4.98675,  4.99727,  4.79981,  5.74915,
     1    4.79981,  3.85379,  6.04394,  6.00795,  5.23986,  5.74915/
C
C LOAD CR=F(SDI) EQUATION SLOPE COEFFICIENT
C
      DATA C1/     -0.02569, -0.02466, -0.01043, -0.00653, -0.01090,
     1   -0.00653, -0.00795, -0.01825, -0.02301, -0.02569, -0.01090/
C
      DATA CRNMLT/11*1.0/
      DATA ICFLG/11*0/
      DATA DLOW/11*0.0/
