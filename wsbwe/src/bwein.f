...
C
   ...
   98 FORMAT (/' ',130('-')//T50,'OPTIONS SELECTED BY INPUT'//
...
C
  ...
  220 FORMAT (/1X,A8,'   END OF BUDWORM OPTIONS.')
C
...
C
  ...
  340 FORMAT (/1X,A8,'   DATA SET REFERENCE NUMBER = ',I5,'; BLANK=',A4,
     >       '; STATUS=',A7/T13,
     >       'MAXIMUM RECORD LENGTH (IGNORED ON SOME MACHINES) = ',I4
     >       /T13,'DATA SET NAME = ',A)
      IF (KODE.EQ.1) WRITE(JOSTND,350)
  350 FORMAT (/T13,'**********   OPEN FAILED   **********')
...
C
  ...
  420    FORMAT (/1X,A8,'   DATA SET REFERENCE NUMBER = ',I5)
         CLOSE (UNIT=I)
      ENDIF
      GOTO 10
...
C
  ...
  520 FORMAT (/1X,A8,'   MANAGEMENT ID= ',A4)
      GOTO 10
C
...
C         DSEEDD = DBLE(DSEEDR)
  ...
  610 FORMAT (/1X,A8,'   DAMAGE MODEL RANDOM NUMBER SEED IS:',
     > F10.0)
  612 FORMAT (/1X,A8,'   * * * ERROR * * *',/12X,
     > 'RANDOM SEED VALUE MUST BE A 5-DIGIT ODD NUMBER. ',
     > 'VALUE PROVIDED WAS:',F10.0,/12X,
     > 'RESEEDING OF RANDOM NUMBER GENERATOR DID NOT OCCUR.')
  614 FORMAT (/1X,A8,'   DAMAGE MODEL RANDOM NUMBER SEED RESET TO:',
     > F10.0)
      GOTO 10
C
...
C
  ...
  710 FORMAT (/1X,A8,'   FIRST YEAR OF PROJECTION IS',I5,'.')
      GOTO 10
C
...
C
  ...
  920 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES=',F4.0,
...
C
 ...
 1020 FORMAT (/1X,A8,'   DATE/CYCLE=',I5,'; SPECIES=',F4.0,
...
C
 ...
 1110 FORMAT (/1X,A8,'   PRINT BUDWORM-CAUSED DAMAGE',
     >        ' OUTPUT.')
      GOTO 10
C
...
C
 ...
 1210 FORMAT (/1X,A8,'   DO NOT PRINT BUDWORM-CAUSED DAMAGE',
     >        ' OUTPUT.')
      GOTO 10
C
...
C
 ...
 1310 FORMAT (/1X,A8,'   PRINT BUDWORM-CAUSED PERIODIC DAMAGE',
     >        ' OUTPUT.')
      GOTO 10
...
C
 ...
 1410 FORMAT (/1X,A8,'   DO NOT PRINT BUDWORM-CAUSED PERIODIC DAMAGE',
     >        ' OUTPUT.')
      GOTO 10
C
...
C
 ...
 1550 FORMAT (/1X,A8,'   CURRENT OUTBREAK (1=Y)=',I2,
...
C
 ...
 1620 FORMAT (/1X,A8,3X,'GEOGRAPHIC LOCATION FOR OUTBREAK.',/12X,
     > 'LOCATION OPTIONS ARE: 1=SOUTHWEST, 2=NORTHWEST, 3=MONTANA',
     > /12X,'OUTBREAK LOCATION SPECIFIED=',I3,' (',A20,')',
     > /12X,'RANDOM NUMBER SEED FOR OUTBREAKS=',F8.1)
      GOTO 10
C
...
C
 ...
 1720 FORMAT (/1X,A8,3X,'THIS OPTION IS CURRENTLY UNDER DEVELOPMENT ',
     & 'AND HAS NO AFFECT.')
...
C
 ...
 1819    FORMAT (/1X,A8,'   WEATHER ',A,';  WEATHER',
     >   ' STATION = (',I3,') ',A,' IN ',A,
...
C
 ...
 1910    FORMAT (/1X,A8,'   NATURAL ENEMY KEYWORD CALLED -- ERROR! ',
     >   'NE = ',I3)
      ELSE
         IF (LKECHO) WRITE(JOSTND,1920) KEYWRD,NE,
     >              (NEMULT(NE,ISTAGE),ISTAGE=1,3)
 1920    FORMAT (/1X,A8,'   NATURAL ENEMY KEYWORD; NE = ',I3,
     >   '  MULTIPLIERS= ',3F7.2)
      ENDIF
      GOTO 10
C
...
C           8) EFFECTS.TAB, UNIT 48   -- not developed ?
 ...
 2005 FORMAT (/1X,A8,3X,'BUDWORM OUTPUT OPTIONS:',7F4.0,/37X,
     > '(1) (2) (3) (4) (5) (6) (7)',/12X,
 ...
 2052           NUMCOL=NUMCOL+1
                IF (NUMCOL.GT.5) THEN
                   WRITE (*,2055) 
 2055              FORMAT(' EEK!  TOO MANY COLUMNS FOR DEFOLIATION ',
     *               'TABLE -- WILL WRITE OVER PREVIOUS COLUMN ')
                   NUMCOL=5
                ENDIF
                DLABS(NUMCOL)=DEFLAB
                CLOSE (JOBWP3)
...
C
 ...
 2120   FORMAT (/1X,A8,'   PARASITISM RATES DO NOT CHANGE DURING THE ',
     >     'OUTBREAK')
      ELSE
        IF(LKECHO)WRITE(JOSTND,2140) KEYWRD
 2140   FORMAT (/1X,A8,'   PARASITISM RATES INCREASE DURING THE ',
     >     'OUTBREAK')
      ENDIF
      GOTO 10
C
...
C
 ...
 2220   FORMAT (/1X,A8,'   LARVAL DEVELOPMENT TIMES DO NOT ',
     >     'CHANGE DURING THE OUTBREAK')
      ELSE
        IF (LKECHO) WRITE(JOSTND,2240) KEYWRD
 2240   FORMAT (/1X,A8,'   LARVAL DEVELOPMENT TIMES INCREASE',
     >     ' DURING THE OUTBREAK')
...
C
 ...
 2320   FORMAT (/1X,A8,'   PUPAL WEIGHTS DO NOT ',
     >     'CHANGE DURING THE OUTBREAK')
      ELSE
        IF (LKECHO) WRITE(JOSTND,2340) KEYWRD
 2340   FORMAT (/1X,A8,'   PUPAL WEIGHTS DECREASE',
     >     ' DURING THE OUTBREAK')
...
C
 ...
 2420 FORMAT (/1X,A8,'   BUDWORM TITLE= ',A72)
      GOTO 10
C
...
C
 ...
 2520    FORMAT (/1X,A8,'   INSECTICIDE TO BE APPLIED IN YEAR ',I4,
     >   /12X,'AT PEAK ',A6,' INSTAR, RESULTING IN ',F5.1,
     >   ' PERCENT MORTALITY')
 ...
 2540    FORMAT (/1X,A8,'   INSECTICIDE TO BE APPLIED WHEN ',
     >   '% DEFOLIATION FIRST RISES ABOVE ',I4,/
     >   12X,'AT PEAK ',A6,' INSTAR, RESULTING IN ',F5.1,
     >   ' PERCENT MORTALITY')
         IF ((LIMITS.EQ.1).AND.LKECHO)WRITE(JOSTND,2550)
 2550    FORMAT (12X,'ONLY ONE SPRAY APPLICATION ALLOWED PER OUTBREAK')
      ENDIF
      GOTO 10
C
...
C
 ...
 2671   FORMAT (/1X,A8,'   STARTING DATES GENERATED FROM FREQUENCY',
     *  ' STATISTICS;',/12X,'OUTBREAKS END WHEN % NEW DEFOLIATION',
     *  ' IS < 10% FOR'/12x,'3 CONSECUTIVE YEARS (5 YEAR MIN.)')

      ELSEIF (IOBOPT.EQ.2) THEN
        IF(LKECHO)WRITE(JOSTND,2672) KEYWRD
 2672   FORMAT (/1X,A8,'   BOTH STARTING AND ',
     >      'ENDING DATES GENERATED FROM STATISTICS')

      ELSEIF (IOBOPT.EQ.3) THEN
        IF(LKECHO)WRITE(JOSTND,2673) KEYWRD
 2673   FORMAT (/1X,A8,'   USER HAS SELECTED OUTBREAK',
 ...
 2675       FORMAT (15x,'OUTBREAK NO. ',I1,':  START= ',I4,
     &         ', END= ',I4)
          ENDIF
 2620   CONTINUE
 ...
 2676   FORMAT (/1X,A8,'   FIRST OUTBREAK SCHEDULED, THEN ',
     &      'GENERAL DEFOLIATOR RUNS ',/12x,
...
C
 ...
 2678   FORMAT (/1X,A8,'   INVALID OUTBREAK SCHEDULING OPTION. FIELD ',
