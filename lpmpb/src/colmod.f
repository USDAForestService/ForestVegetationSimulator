      ...
      IMPLICIT NONE
C----------
C  **COLMOD        DATE OF LAST REVISION:  07/02/10
C----------
C
...
C
   ...
   37 FORMAT ('1MOUNTAIN PINE BEETLE RATE OF LOSS MODEL :  OUTBREAK',
     >       ' IN YEAR = ',I4,' FOR ',I2,' YEARS.',/)

      WRITE(JOMPB,40)
   40 FORMAT (20X,'TREES/ACRE OF LIVE LODGEPOLE PINE BY DBH CLASS',T86,
     &        ' TOTAL    TOTAL')
   ...
   42 FORMAT (' START ',10F7.2,5X,2F9.2)
     ...
     &                      TGREEN(L), TDEAD(L)
   50    FORMAT (1X,I4,2X,10F7.2,5X,2F9.2)
  100 CONTINUE
...
C
  ...
  116      CONTINUE
     ...
     >                      MDGTOT, MDDTOT
  117      FORMAT (1X,I4,1X,A26,5X,16I5)
  120   CONTINUE
      ENDIF
