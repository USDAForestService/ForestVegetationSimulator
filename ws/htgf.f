...
C
C    GS AND RW USE NEW REALTIONSHIP FROM CASTLE 2019.
C----------
C
      DATA MXHTG1/
...
C  DETERMINE WHICH SPECIES TO PRCOESS FROM CA
        SELECT CASE(ISPC)

C----------
C  PROCESS RW AND GS WHICH USE THE FOLLOWING FUNCTIONAL FORM
C  
C  HI = EXP(X)
C  X = B1 + D1^2 + LOG(D1) + LOG(SI) + LOG(DGLT) + LOG(H)
C
C  WHERE
C  HI = 10 YEAR HEIGHT GROWTH
C  D1 = DIAMETER AT BREAST HEIGHT
C  SI = SITE INDEX (BASE AGE 50)
C  DGLT = 10-YEAR DIAMETER GROWTH
...
C----------
C  PROCESS ALL OTHER SPECIES WHICH USE CA EQUATIONS
C----------
        CASE DEFAULT
