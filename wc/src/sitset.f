      ...
      IMPLICIT NONE
C----------
C  **SITSET--WC   DATE OF LAST REVISION:  07/30/14
C----------
C
...
C
      ...
      REAL FORMAX,RSI,RSDI,AG,SINDX,SI,BADEF
C----------
C  DATA STATEMENTS
C----------
...
C----------
      ...
      KNTECO=0
      NTOHI=0
   10 CALL ECOCLS (PCOM,ASPEC,RSDI,BADEF,RSI,ISFLAG,NUM,INDEX,ISEQ)
      KNTECO=KNTECO+1
      IF(DEBUG)WRITE(JOSTND,*)'AFTER ECOCLS,PCOM,ASPEC,RSDI,BADEF,RSI,',
     &'ISFLAG,NUM,INDEX,ISEQ = ',PCOM,ASPEC,RSDI,BADEF,RSI,ISFLAG,
     &NUM,INDEX,ISEQ
C----------
...
C----------
   ...
   80 CONTINUE
C-----------
C LOAD THE DEFAULT BAMAX IF ONE HAS NOT BEEN SET BY THE USER
C IF A BADEF DOES NOT EXIST FOR THE P.A. THEN 999 IS RETURNED
C SET FLAG SO BAMAX DOESN'T GET RESET FROM SDIMAX IN SDICAL
C----------
      IF(BAMAX.LE.0) BAMAX=SDIDEF(ISISP)*(PMSDIU/100.)*0.54542
      IF(DEBUG)WRITE(JOSTND,*)' IN SITSET, SETTING DEFAULT BAMAX, ',
     &'BAMAX,BADEF= ',BAMAX,BADEF
C
      IF(BAMAX.LT.BADEF .AND. BADEF.LT.900)BAMAX=BADEF
      LBAMAX=.TRUE.
C      
      IF(DEBUG)WRITE(JOSTND,*)' FINAL DEFAULT BAMAX= ',BAMAX
C
      DO 92 I=1,15
      J=(I-1)*10 + 1
      JJ=J+9
