...
C     SPECIES OF EACH RECORD. CALLS DBSFMBMCMPCUT TO WRITE CUT TREE
C     BIOMASS COMPONENT DATABASE TABLE
C
C     CALLED FROM -- CUTS
C     CALLS FMSSEE
C           CWD3
...
C
      INTEGER I, ISZ, IDC, NR, NC, J, K, IT, IWHO
...
C
C     IWHO=FLAG TO TELL THE FMCBIO ROUTINE THAT CALL IS FROM
C     FMSCUT
...
C     BE NEGATIVE, SO WE SET IT TO 0.
   ...
   10 FORMAT(' FMSCUT, TONRMH= ',F12.3)
      ...
      IF(IFMBMCMPCUT.GT.0)CALL DBSFMBMCMPCUT(IY(ICYC))
