...
CSB          IF (JYR .NE. IYR) GO TO 35
          CALL OPDONE (I,IYR)
          ICRPTB = MAX(1,IYR)
          ICRPTE = MAX(ICRPTB+1,JYR+INT(PRMS(1)))
          ICRPTI = MAX(1,INT(PRMS(2)))
   35   ENDDO
      ENDIF
C
...
C     ON THE REPORTING INTERVAL
      ...
      IF (MOD((IYR-ICRPTB),ICRPTI).NE.0) GOTO 900
...
C     JUST A COMPLEMENT-MULTIPLIER. CLCWD PROVIDES POSSIBLE
C     CLIMATE-SENSITIVE CHANGE TO DECAY RATE. SEE SUB FMCWD
C     FOR OTHER CWD EXAMPLES.
C
  900 IF (LDCAY) THEN
        X = 1.0
!       X = CLCWD(Q10CWD(12),REFMATCWD(12)) - stub for Climate-decay (in development)
        BIOROOT = BIOROOT * (1.0 - CRDCAY*X)
      ELSE
        BIOROOT = 0.0
      ENDIF
