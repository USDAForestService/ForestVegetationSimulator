      ...
      IMPLICIT NONE
C----------
C  **SITSET--CR/M   DATE OF LAST REVISION:  02/09/12
C----------
...
C----------
      WRITE(JOSTND,10)
   10 FORMAT(/1X,'MODTYPE    KEYWORD NOW BEING PROCESSED')
C----------
C  DECODE MODEL TYPE
C----------
 ...
 200    FORMAT(T13,'MODEL TYPE FOR THIS PROJECTION IS ',I4,'. ',A26)
      ELSE
        IMODTY=DEFMT(IFOR)
        IF(IMODTY .EQ. 1) LABEL='SOUTHWEST MIXED CONIFERS  '
        IF(IMODTY .EQ. 2) LABEL='SOUTHWEST PONDEROSA PINE  '
   ...
   14   FORMAT(' ',T13,'MODEL TYPE NOT RECOGNIZED. BEING SET TO',I4,
