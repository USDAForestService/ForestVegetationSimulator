...
C     
C     IN THE I=1,ROWS LOOP BELOW, ONLY (ISP, ITPA, IBA, ICF & IBF) ARE NEED;
C     OTHER ARGUMENTS (IDIST(*:1), SIGLEVEL & LABELS(1)) ARE UNUSED IN 
C     SUBROUTINE DBSSTATS AND ARE USED TO KEEP THE CALL CALL CONSISTENT
C     FOR THE 2 KINDS OF TABLES CREATED BY DBSSTATS. - dr/essa
C
      DO I=1,ROWS
        CALL DBSSTATS(SPECCD(I),TPA(I),BAREA(I),CFVOL(I),
     &   BFVOL(I),IDIST(1,1),IDIST(2,1),IDIST(3,1),
     &   IDIST(4,1),SIGLEVEL,IDIST(5,1),IDIST(6,1),IDIST(7,1),
     &   IDIST(8,1),LABELS(1),1,IYEAR)
      ENDDO
      K=3
      DO I=1,3
