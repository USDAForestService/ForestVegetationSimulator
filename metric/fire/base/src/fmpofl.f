...
C  **FMPOFL FIRE-BASE/M DATE OF LAST REVISION:  01/09/08
...
C
      INTEGER IYR,JDO,NTODO,NPRM,IACTK,JYR,IK,JPOTFL,I,IDPL,DBSKODE, K
...
C     CHECK TO SEE IF THE POT. FLAME REPORT HAS BEEN REQUESTED.
C     SET IPFLME TO THE LAST YEAR THAT THE REPORT WILL BE GENERATED.
C     RETREIVE THE UNIT NUMBER.

      JDO = 0
      CALL OPFIND(1,MYACT,NTODO)
      DO 5 JDO = 1,NTODO

C        LOOP OVER THE POSSIBLE ACTIONS UNTIL WE FIND ONE THAT
C        IS FOR THIS YEAR, IF ANY.
         ...
         IPFLME = JYR + PRMS(1)
         IPFSTP = PRMS(2)
         IF (IPFSTP.EQ.0) IPFSTP=1

C        FOR CONSTISTENCY, PUT THE OPDONE CALL WHERE THE GETTING
C        OF THE OPTIONS IS DONE, RATHER THAN THE DOING OF THE PRINTING.
...
C     PERIOD AND THAT IT IS A VALID YEAR (IF WE ARE USING A PRINTING INTERVAL).
      ...
      IF (DEBUG) WRITE(JOSTND,7) IPFLMB,IPFLME,IPFSTP
    7 FORMAT(' IN FMPOFL IPFLMB=',I5,' IPFLME=',I5,' IPFSTP=',I5)
...
C
      ...
      IF (MOD((IYR-IPFLMB),IPFSTP) .NE. 0) RETURN
   10 CONTINUE
