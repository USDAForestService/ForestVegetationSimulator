     ...
     1                  LBWEGO,LCVATV,LBGCGO)
      IMPLICIT NONE
C----------
C  **GRADD--BASE   DATE OF LAST REVISION: 07/23/08
C----------
C
...
C
      REAL SPCNT(MAXSP,3)
      INTEGER IPMODI,I,IS,IYR1,IYR2,IFIRES,IDT,ISTAT,KODE,NPRMS,ILE,IM
      REAL SCALE,D,BARK,BRATIO,DDS
C
      IF (DEBUG) WRITE (JOSTND,10) NPLT,ICYC
   10 FORMAT (/' IN GRADD, NPLT=',A26,'; ICYC=',I2)
C----------
...
C----------
C  ORGANON: KILL ANY ALTERATION OF THE PREDICTED INCREMENTS.
C----------
c$$$C
c$$$C     SCALE DIAMETER GROWTH TO A FINT-YEAR BASIS.
c$$$C
c$$$      IF (ITRN.GT.0) THEN
c$$$
c$$$C         SCALE=FINT/YR
...
c$$$C         IS=ISP(I)
c$$$C         D=DBH(I)
c$$$C         BARK=BRATIO(IS,D,HT(I))
c$$$C         IF(DG(I) .GT. 0.) THEN
c$$$C           DDS=(DG(I)*(2.0*BARK*D+DG(I)))*SCALE
c$$$C           DG(I)=SQRT((D*BARK)**2+DDS)-BARK*D
c$$$C         ENDIF
c$$$         IF (DG(I).LT.0.0) DG(I)=0.0
c$$$   30    CONTINUE
c$$$
c$$$      ENDIF
...
C
   ...
   96 FORMAT(' CALLING CLAUESTB, CYCLE=',I2)
      CALL CLAUESTB
C
...
C
      IF (DEBUG) WRITE (JOSTND,110) ICYC,LBGCGO
  110 FORMAT(' CALLING CROWN, CYCLE=',I2,'  LBGCGO= ',A8)
      CALL CROWN
C
...
C----------
C  ORGANON: KILL ANY ALTERATION OF THE PREDICTED INCREMENTS.
C     YOU MIGHT HAVE TO GUT THIS SECTION TOO...
C     BUT VERIFY BEFORE YOU DO THAT...
C----------
