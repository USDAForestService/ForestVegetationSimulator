...
C     THIS FILE IS MODIFIED FROM THE ORIGINAL FILE TO CALL 
C     THE VOLCALC SUBROUTINE IN THE ORGVOL DYNAMIC LINK LIBRARY
C     THE SECTIONS OF THE CODE THAT ARE MODIFIED TO LOAD AND CALL
C     THE ORGANON ORGVOL.DLL ARE DENOTED WITH THE FOLLOWING
C     COMMENT BLOCK (EXAMPLE)
C
C----------
C  ORGANON: LOAD THE DYNAMIC LINK LIBRARY, VERIFY THAT THE 
C           SUBROUTINE IS PRESENT, AND ASSIGN THE POINTER TO THE 
...
C     DEFINES LOGTA, LOGML, LOGLL, AND VERSION
C----------
      INCLUDE 'ORGANON.F77'
C
C
...
C----------
C  ORGANON: IMPORT THE VOLCAL FUNCTION FROM THE ORGVOL.DLL
C----------
C      DLL_IMPORT VOLCAL
!DEC$ ATTRIBUTES DLLIMPORT, C, DECORATE, ALIAS : "VOLCAL" :: VOLCAL


c$$$      interface
c$$$      SUBROUTINE VOLCAL(  VERSION,SPP,
c$$$     1                    CFTD,CFSH,
c$$$     2                    LOGLL,LOGML,LOGTD,LOGSH,LOGTA,
c$$$     3                    DBH,HT,CR,
c$$$     4                    VERROR,TERROR,VWARNING,TWARNING,IERROR,
c$$$     5                    CFVOL,BFVOL)
c$$$      !DEC$ ATTRIBUTES DLLIMPORT:: VOLCAL
c$$$      INTEGER*4   VERSION         ! ORGANON.VERSION
c$$$      INTEGER*4   SPP             ! ORGANON (FIA) SPECIES CODE
c$$$      REAL*4      CFTD            ! CUBIC VOLUME TOP DIAMETER, IN INCHES
c$$$      REAL*4      CFSH            ! CUBIC VOLUME, STUMP HEIGHT, IN FEET
c$$$      REAL*4      LOGLL           ! LOG LENGTH (TARGET), IN FEET
c$$$      REAL*4      LOGML           ! LOG MINIMUM LENGTH, IN FEET
c$$$      REAL*4      LOGTD           ! SCRIBNER VOLUME, TOP DIAMETER, IN INCHES
c$$$      REAL*4      LOGSH           ! SCRIBNER VOLUME, STUMP HEIGHT, IN FEET
c$$$      REAL*4      LOGTA           ! SCRIBNER VOLUME, LOG TRIM, IN INCHES
c$$$      REAL*4      DBH             ! DBH OF THE STEM, IN INCHES
c$$$      REAL*4      HT              ! TOTAL HEIGHT OF THE STEM, IN FEET
c$$$      REAL*4      CR              ! CROWN RATIO OF THE STEM, UNITLESS
...
c$$$      REAL*4      CFVOL           ! MERCH. STEM CUBIC VOLUME, IN CUBIC FEET
c$$$      REAL*4      BFVOL           ! MERCH. STEM SCRIB. BOARD FT. VOLUME, IN BOARD FEET
c$$$	end subroutine
c$$$	end interface
...
C----------
C  ORGANON: CLEAR THE WARNING/ERROR FLAGS
C----------

C----------
C  ORGANON: CALL THE ORGVOL.DLL VOLCALC SUBROUTINE
C----------
C      CALL VOLCAL(      VERSION,SPP,
...
C----------
         DO Z = 1, 5
            IF( ORGVOL_TERROR(Z) .EQ. 1 .OR. 
     &           ORGVOL_VERROR(Z) .eq. 1 ) THEN
...
C-----------
      ...
      VM      = VM
      VMAX    = VM
!      CFV(I)  = VM   !! THIS GETS UPDATED AFTER WK5 IS POPULATED.
                      !!  OTHERWISE THE ACCRETION CODE DOESN'T FUNCTION CORRECTLY.
      BFV(I)  = BBFV
...
C  CALL ECON EXTENSION TO PROCESS VOLUME INFORMATION
C----------
      IF (BFV(I) .GT. 0.0) CALL ECVOL(ISPC,IT,LOGDIA,LOGVOL,.FALSE.)
