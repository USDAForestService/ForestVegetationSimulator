...
C**********************************************************************
C  **BRIN         DATE OF LAST REVISION:  05/15/2006
C----------------------------------------------------------------------
C  Purpose:
...
C.... Output for PRUNE keyword.
  ...
  110 FORMAT(/1X,A8,'   PRUNING SCHEDULED IN CYCLE/YEAR ',I4,
     &  ';  SUCCESS RATE = ',F6.2,';')
      IF(INT(ARRAY(3)).EQ.1 .AND. INT(ARRAY(4)).EQ.1) THEN
         WRITE (JOSTND,111)
  111    FORMAT(T13,'BOTH PRUNABLE INFECTED AND CLEAN TREES HAVE',
     &     ' BEEN TARGETED FOR PRUNING')
      ELSE IF(INT(ARRAY(3)).EQ.1) THEN
         IF(LKECHO)WRITE(JOSTND,112)
  112    FORMAT(T13,'PRUNABLE INFECTED TREES HAVE BEEN TARGETED',
     &     ' FOR PRUNING')
      ELSE IF(INT(ARRAY(4)).EQ.1) THEN
         IF(LKECHO)WRITE(JOSTND,113)
  113    FORMAT(T13,'CLEAN TREES HAVE BEEN TARGETED FOR PRUNING')
      ENDIF

      IF(INT(PRMS(4)) .EQ. 1) THEN
         IF(LKECHO)WRITE(JOSTND,114)
  114    FORMAT(T13,'PATHOLOGICAL PRUNING IS SPECIFIED; THEREFOR,',/,
     &     T13,'CROWN BASE HEIGHTS WILL NOT BE CHANGED WHEN PRUNED.')
      ELSE
...
C.... Output for PRNSPECS keyword.
  ...
  210 FORMAT(/1X,A8,'   PRUNE PARAMETERS CHANGED IN DATE/CYCLE ',I4,
     &   ';  MAXIMUM PRUNABLE HEIGHT AS A PROPORTION OF TOTAL',
     &   ' TREE HEIGHT = ',F4.2,';',/T13,'MAXIMUM ABSOLUTE PRUNE',
     &   ' HEIGHT = ',F5.1,' FEET;  MINIMUM DISTANCE OUT FOR PRUNABLE',
     &   ' CANKER = ',F4.1,' INCHES;',/T13,'MINIMUM DISTANCE OUT FOR',
...
C.... Output for EXCISE keyword.
  ...
  310 FORMAT(/1X,A8,'   EXCISING SCHEDULED IN DATE/CYCLE ',I4,
     &   ';  SUCCESS RATE = ',F4.2)
...
C 320 FORMAT(/1X,A8,'   EXCISE INITIATED PRUNING SCHEDULED IN CYCLE',
C    &       '/YEAR ',I4,';  SUCCESS RATE = ',F6.2,';',/,
C    &       T13,'ONLY PRUNABLE TREES HAVE BEEN TARGETED FOR PRUNING')
...
C.... Output for EXSPECS keyword.
  ...
  410 FORMAT(/1X,A8,'   EXCISE PARAMETERS CHANGED IN DATE/CYCLE ',I4,
     &  ';  MINIMUM TREE DBH NECESSARY FOR EXCISING = ',
     &  F5.1,' INCHES;',/T13,'MAXIMUM ABSOLUTE EXCISE HEIGHT = ',
     &  F5.1,' FEET;  MAXIMUM PERCENT GIRDLE = ',F5.1,'%;',/T13,
     &  'PERCENT GIRDLING WHICH CAUSES MORTALITY OR TOP KILL = ',F5.1,
     &  '%;'/T13,'MINIMUM BOLE CANKER HEIGHT NECESSARY FOR EXCISING = '
     &  ,F5.1,' INCHES')
      GO TO 90
...
C....    Output for RIBES keyword when date/cycle = 0.
  ...
  510    FORMAT(/1X,A8,'   STAND RUST INDEX INITIALLY SET TO ',F7.5,';',
     &      /T13,'CALCULATED FROM NEW BUSHES PER ACRE VALUES SHOWN '
     &      'BELOW:',/T13,'RIBES H.   NEW BUSHES/ACRE = ',F6.1,';',
     &      /T13,'RIBES L.   NEW BUSHES/ACRE = ',F6.1,';',
     &      /T13,'RIBES V.   NEW BUSHES/ACRE = ',F6.1)
      ELSE
...
C....    Output for RIBES keyword when date/cycle not 0.
  ...
  530    FORMAT(/1X,A8,'   RUST INDEX TO BE MODIFIED IN DATE/CYCLE ',
     &      I4,' BASED ON CHANGE IN NUMBER OF RIBES BUSHES;',
     &      /T13,'RIBES H.   OLD BUSHES/ACRE = ',F6.1,
     &      ';  NEW BUSHES/ACRE = ',F6.1,';',
     &      /T13,'RIBES L.   OLD BUSHES/ACRE = ',F6.1,
     &      ';  NEW BUSHES/ACRE = ',F6.1,';',
     &      /T13,'RIBES V.   OLD BUSHES/ACRE = ',F6.1,
     &      ';  NEW BUSHES/ACRE = ',F6.1)
      ENDIF
      GO TO 90
...
C.... Output for INACT keyword.
  ...
  610 FORMAT(/1X,A8,'   CANKER INACTIVATION RATES TO BE CHANGED IN',
     &   ' DATE/CYCLE ',I4,';',/T13,'NEW VALUE FOR BRANCH CANKERS = ',
...
C.... End of Blister Rust keywords.
  ...
  710 FORMAT(/1X,A8,'   END OF BLISTER RUST KEYWORDS')
...
C.... I5 is the BR host stock type
  ...
  910 FORMAT(/1X,A8,'   CHANGE ANNUAL GROWTH RATES FOR CANKERS.')
c    &  ,/,
c    &  T13,'FIELDS 1 AND 2 FOR SPECIES AND STOCK TYPE NOT YET ACTIVE,')
...
C.... Vaildate species code
  ...
  914    FORMAT(/1X,A8,'   ***** INVALID SPECIES CODE ',I3,'. *****')
         GO TO 90
      ELSE IF(I4 .EQ. 0 .AND. I3 .NE. 0) THEN
         WRITE(JOSTND,915) KEYWRD, I3, NSP(I3,1)(1:2)
  915    FORMAT(/1X,A8,'   ***** NON-HOST SPECIES CODE ',
     &          I3,' - ',A,'. *****')
         GO TO 90
      ENDIF
...
C.... Vaildate stock type.
  ...
  917    FORMAT(/1X,A8,'   ***** INVALID STOCK TYPE ',I1,'. *****')
         GO TO 90
        I5=INT(ARRAY(2))
      ENDIF
      ...
      IF(LNOTBK(3)) THEN
        ...
        ELSE
          WRITE(JOSTND,918)
  918     FORMAT(T13,'*** INVALID BOLE CANKER GROWTH RATE. VALID ',
     &    'RANGE IS 0.1 TO 10.0 -- DEFAULT VALUE(S) RETAINED. ***')
      ...
      IF(LNOTBK(4)) THEN
        ...
        ELSE
          WRITE(JOSTND,919)
  919     FORMAT(T13,'*** INVALID BRANCH CANKER GROWTH RATE. VALID ',
     &    'RANGE IS 0.1 TO 10.0 -- DEFAULT VALUE(S) RETAINED. ***')
        ENDIF
      ENDIF
...
C....     Changing 1 species, 1 stock type
  ...
  925 FORMAT(T13,'ALL BLISTER RUST HOST SPECIES AND STOCK TYPES',
     &  ' CANKER GROWTH RATES SET AS FOLLOWS:')
  926 FORMAT(T13,'ALL BLISTER RUST HOST SPECIES STOCK TYPE ',I1,
     &  ' CANKER GROWTH RATES SET AS FOLLOWS:')
  927 FORMAT(T13,'ALL STOCK TYPES OF BLISTER RUST HOST SPECIES ',A3,
     &  ' CANKER GROWTH RATES SET AS FOLLOWS:')
  928 FORMAT(T13,'BLISTER RUST HOST SPECIES ',A3,' STOCK TYPE ',I1,
     &  ' CANKER GROWTH RATES SET AS FOLLOWS:')
...
C.... Now write canker growth rate values that have been set.
  ...
  930 FORMAT(T13,'BOLE CANKER WIDEST HORIZONTAL WIDTH GROWTH ',
     &  'RATE (CM) = ',F6.3,/,
     &  T13,'BRANCH CANKER RADIAL GROWTH RATE (CM) = ',F6.3)
      GO TO 90
...
C.... Output for BRTLST keyword.
 ...
 1020    FORMAT(/1X,A8,'   BLISTER RUST TREE LIST WILL BE WRITTEN TO',
     &      ' UNIT NUMBER ',I2,' FOR ALL CYCLES;',
     &      /T13,'TREELIST FILENAME = ',A50)
      ELSE
         IF(LKECHO)WRITE(JOSTND,1021) KEYWRD,IDTOUT,IDT,TNAME
 1021    FORMAT(/1X,A8,'   BLISTER RUST TREE LIST WILL BE WRITTEN TO',
...
C.... Output for BRCLST keyword.
 ...
 1120    FORMAT(/1X,A8,'   BLISTER RUST CANKER LIST WILL BE WRITTEN TO',
     &      ' UNIT NUMBER ',I2,' FOR ALL CYCLES;',
     &      /T13,'CANKER LIST FILENAME = ',A50)
      ELSE
         IF(LKECHO)WRITE(JOSTND,1121) KEYWRD,IDCOUT,IDT,CNAME
 1121    FORMAT(/1X,A8,'   BLISTER RUST CANKER LIST WILL BE WRITTEN TO',
     &      ' UNIT NUMBER ',I2,' FOR DATE/CYCLE ',I4,';',
     &      /T13,'CANKER LIST FILENAME = ',A50)
      ENDIF
      GO TO 90
...
C.... Keyword to change the default format for reading canker data.
 ...
 1210 FORMAT(/1X,A8,'   THE FORMAT BELOW WILL BE USED FOR READING ',
     &   'CANKER DATA. THE ORDER OF THE VARIABLES IS:',/,T13,
...
C     Vaildate species code
 ...
 1305    FORMAT(/1X,A8,'   ***** INVALID SPECIES CODE ',I3,'. *****')
         GO TO 90
      ELSE IF(BRSPM(KSP) .EQ. 0) THEN
         WRITE(JOSTND,1306) KEYWRD, KSP, NSP(KSP,1)(1:2)
 1306    FORMAT(/1X,A8,'   ***** NON-HOST SPECIES CODE ',
     &          I3,' - ',A,'. *****')
         GO TO 90
      ENDIF
...
C        Schedule activity.
 ...
 1313      FORMAT(/1X,A8,'   ***** Activity Scheduling Error *****')
           GO TO 90
         ENDIF
      ENDIF
...
C
 ...
 1316 FORMAT(/1X,A8,'   STAND DEVIATION FACTORS INITIALIZED FOR')
 1317 FORMAT(/1X,A8,'   STAND DEVIATION FACTORS WILL BE RESET AT ',
     &       'CYCLE/YEAR ',I4,' FOR')
 1320 FORMAT(T13,'TREE SPECIES = ',A,/,T13,'STOCK TYPE 1 = ',F5.3,
     &      /,T13,'STOCK TYPE 2 = ',F5.3,/,T13,'STOCK TYPE 3 = ',F5.3,
     &      /,T13,'STOCK TYPE 4 = ',F5.3)
 1321 FORMAT(T13,'*** INVALID DEVIATION FACTOR ENTERED: ',F5.3,
...
C.... Output for RUSTINDX keyword.
 ...
 1410 FORMAT(/1X,A8,'   STAND RUST INDEX INITIALLY SET TO ',F7.5,';')
 1412 FORMAT(T13,'RUST INDEX WILL BE CALCULATED FROM BASAL AREA AT ',
     &   'INITIALIZATION AND USED THROUGHOUT THE SIMULATION')
 1413 FORMAT(T13,'RUST INDEX WILL BE CALCULATED FROM BASAL AREA ',
     &   'EACH CYCLE AND APPLIED TO ALL TREES.')
 1414 FORMAT(T13,'RUST INDEX BASED ON STAND AGE GAUSSIAN FUNCTION ',
     &   'EACH CYCLE AND APPLIED TO ALL TREES')
 1415 FORMAT(T13,'RUST INDEX SHOWN ABOVE WILL BE USED ',
     &   'THROUGHOUT THE SIMULATION')
 1416 FORMAT(T13,'RIBES SPECIES PROPORTIONS OF POPULATION WHICH WILL ',
     &   'BE USED FOR RUST INDEX CALCULATION ARE:',/T16,
     &   'RIBES H. = ',F4.2,';  RIBES L. = ',F4.2,';  RIBES V. = ',F4.2)
 1417 FORMAT(T13,'THE PARAMETERS USED IN EQUATION OF RI ASSIGNMENT ',
     &   'METHOD SELECTED ARE AS FOLLOWS.',
     &   /,T16,'MINIMUM RUST INDEX VALUE: ',F11.9,
     &   /,T16,'MAXIMUM RUST INDEX VALUE: ',F11.9,
     &   /,T16,'STAND AGE WHERE RI PEAKS: ',F10.5,
     &   /,T16,'CURVE SHAPE DEFINED BY:   ',F10.7)
 1418 FORMAT(T13,'RUST INDEX BASED ON STAND AGE LOG FUNCTION',
     &   'EACH CYCLE AND APPLIED TO ALL TREES.')
...
C.... Output for BRSEED keyword.
 ...
 1510 FORMAT(/1X,A8,'   RANDOM SEED FOR BLISTER RUST MODEL = ',F10.0)
      GO TO 90
...
C.... Output for STOCK keyword.
 ...
 1620 FORMAT(/1X,A8,'   MIX OF STOCK TYPES TO BE CHANGED IN ',
     & 'DATE/CYCLE ',I4,';',/T13,'TREE SPECIES=',I3,'; STOCK TYPE = ',
...
C.... Output for CANKDATA keyword.
 ...
 1720    FORMAT(/1X,A8,'   CANKER DATA PROVIDED IN CENTIMETERS',
     &      ' (DISTANCE UP AND DISTANCE OUT);')
      ELSE
         WRITE (JOSTND,1725) KEYWRD
 1725    FORMAT(/1X,A8,'   CANKER DATA PROVIDED IN FEET (DISTANCE UP)',
     &      ' AND INCHES (DISTANCE OUT);')
      ENDIF

      IF(CKINIT)THEN
         IF(LKECHO)WRITE(JOSTND,1730)
 1730    FORMAT(T13,'CANKER DATA WILL BE RANDOMLY GENERATED AT',
...
C.... supplemental record (directly following the CANKDATA keyword).
 ...
 1705    FORMAT(T13,'CANKER DATA WILL BE READ FROM SUPPLEMENTAL',
...
C....    Open the canker data file.
 ...
 1715    FORMAT(T13,'CANKER DATA WILL BE READ FROM UNIT NUMBER ',I2,
     &      ';',/T13,'CANKER DATA FILENAME = ',A80)
      ENDIF
...
C.... Output for BROUT keyword.
 ...
 1820 FORMAT(/1X,A8,'   BLISTER RUST SUMMARY AND DBH CLASS ',
     &      'STATISTICS TABLES CONTROL.')
      IF(LKECHO .AND. LBRSUM)WRITE(JOSTND,1821)
 1821 FORMAT(T13,'STAND SUMMARY STATISTICS WILL BE PRINTED.')
      IF(LKECHO .AND. LBRDBH)WRITE(JOSTND,1822)
 1822 FORMAT(T13,'STAND DBH CLASS STATISTICS WILL BE PRINTED.')
      IF(LKECHO .AND. .NOT. LBRDBH
     &          .AND. .NOT. LBRSUM)WRITE(JOSTND,1823)
 1823 FORMAT(T13,'BOTH STAND DBH CLASS AND SUMMARY STATISTICS '
     &      'IS SUPPRESSED.')
