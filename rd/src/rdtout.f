      SUBROUTINE RDTOUT
C----------
C  **RDTOUT      LAST REVISION:  MARCH 5, 1995
C----------
C
...
C
 ...
 1000 FORMAT (/1X,130('-'))
      WRITE (JOSTND,1010)
 1010 FORMAT (/1X,42X,'SUMMARY OF DISEASE INFORMATION READ IN ON',
     &       ' INPUT')
      WRITE (JOSTND,1000)

      DO 90 IDI=MINRR,MAXRR
         WRITE (JOSTND,1020) DITYPE(IDI)
 1020    FORMAT (/12X,'DISEASE TYPE: ',A1)

         IF (RRTINV) THEN
            WRITE (JOSTND,1030) RINNF(IDI), RINUF(IDI), RISTU(IDI)
 1030       FORMAT (12X, 'NUMBER OF INFECTED TREE RECORDS IN ',
     &          'DISEASE PATCHES=  ', F6.0/
     &          12X, 'NUMBER OF UNINFECTED TREE RECORDS IN ',
     &          'DISEASE PATCHES=', F6.0/
     &          12X, 'NUMBER OF INFECTED STUMP RECORDS IN ',
     &          'DISEASE PATCHES= ', F6.0)
         ENDIF
         WRITE (JOSTND,1040)
         WRITE (JOSTND,1050)
 ...
 1040    FORMAT (/,12X, '        NUMBER OF INFECTED STUMPS         ')
 1050    FORMAT (/,12X, 'SIZE CLASS      RESINOUS      NON-RESINOUS')
 1060    FORMAT (12X,   '------------------------------------------')
     ...
     &            PROBDA(IDI,1,J,1), PROBDA(IDI,2,J,1)
              ENDIF 
            ENDIF  
 1070       FORMAT (13X,I2,'-',I3,'"',6X,F5.0,10X,F5.0)
 1080       FORMAT (13X,'  >',I3,'"',6X,F5.0,10X,F5.0)
