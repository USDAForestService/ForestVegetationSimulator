...
C
     ...
     &    ISPC,IREGN,FORST,VEQNNC(ISPC),TOPDIB,STMP(ISPC),D,H,
     &    DBT,BARK
C
...
C----------
     ...
     &    ',BARK*100.,IFC,TOPDIB,H,SCALEN,DBTBH,X01,CTYPE,IERR= ',
     &    VEQNNC(ISPC),FORST,D,BARK*100.,IFC,TOPDIB,H,
     ...
     &           LOGDIA,LOGLEN,LOGVOL,LOGST,NLOGMS,NLOGTW,1,1,IERR)
          IF(DEBUG)WRITE(16,*)' AFTER BLMVOL-VEQNNC(ISPC),TOPDIB,',
     &    'H,X01,D,IFC,LOGST,NLOGMS,NLOGTW,IERR= ',
     &    VEQNNB(ISPC),TOPDIB,H,X01,D,'F',IFC,LOGST,NLOGMS,NLOGTW
          IF(DEBUG)WRITE(JOSTND,*)' TVOL= ',TVOL
        ENDIF
      ENDIF
C----------
...
C
     ...
     &      'TOPDIB,H,X01,D,F,IFC,LOGST,NLOGMS,NLOGTW= ',
     &      VEQNNB(ISPC),TOPDIB,H,X01,D,'F',IFC,LOGST,NLOGMS,NLOGTW
            IF(DEBUG)WRITE(JOSTND,*)' TVOL= ',TVOL
          ENDIF
          TVOL(1)=TVOL1
...
C
        IF(DEBUG)WRITE(JOSTND,*)' CALLING PROFILE CF ISPC,ARGS = ',
     &   ISPC,IREGN,FORST,VEQNNC(ISPC),TOPDIB,STMP(ISPC),D,H,DBT,BARK
C
        CALL PROFILE (IREGN,FORST,VEQNNC(ISPC),TOPDIB,X01,STMP(ISPC),D,
     &  HTTYPE,H,I01,X02,X03,X04,X05,X06,X07,X08,X09,I02,DBT,BARK*100.,
     &  LOGDIA,BOLHT,LOGLEN,LOGVOL,TVOL,I03,X010,X011,I1,I1,I1,I04,
...
C
          IF(DEBUG)WRITE(JOSTND,*)' CALLING PROFILE BF ISPC,ARGS = ',
     &    ISPC,I1,FORST,VEQNNB(ISPC),TOPDIB,BFSTMP(ISPC),D,H,
     &    DBT,BARK
C
...
C
          IF(DEBUG)WRITE(JOSTND,*)' AFTER PROFILE BF TVOL= ',TVOL
        ELSE
          IF(DEBUG)WRITE(JOSTND,*)' CALLING R5HARV BF ISPC,R5EQN,D,H,',
     &    'TOPDIB,I1= ',ISPC,VEQNNB(ISPC),D,H,TOPDIB,I1
