      ...
      IMPLICIT NONE
C----------
C  **CROWN--CA    DATE OF LAST REVISION:  03/31/11
C----------
...
C----------
      ...
      REAL PRM(5)
      INTEGER ICRFST,JJ,NTODO,I,NP,IACTK,IDATE,IDT,ISPCC,IGRP,IULIM
      INTEGER IG,IGSP,J1,ISPC,I1,I2,JSPC,I3,IITRE,ICRI
      REAL RELSDI,ACRNEW,A,B,C,D,H,SCALE,X,RNUMB,CRHAT,PCTHAT
      REAL DIFF,CHG,PDIFPY,CRLN,CRMAX,HN,HD,CL,TPCT,TPCCF,CR
C
...
C
 ...
 9001 FORMAT('0 IN CROWN 9001 WRITE ISPC,SDIAC,ORMSQD,RELSDI,ACRNEW,A,B,
...
C----------
      IF(ICRFST.EQ.0 .AND. ICR(I).GT.0)GOTO 60
C----------
...
C----------
      IF (ICRFST .EQ. 0) GOTO 40
      IF (ICR(I).GE.0) GO TO 40
      ICR(I)=-ICR(I)
      IF (DEBUG) WRITE (JOSTND,35) I,ICR(I)
   35 FORMAT ('0 ICR(',I4,') WAS CALCULATED ELSEWHERE AND IS ',I4)
      GOTO 60
   40 CONTINUE
      D=DBH(I)
      H=HT(I)
...
C----------
      IF(D.LT.1.0 .AND. ICRFST .EQ. 0) GO TO 58
C----------
...
C----------
   50 CONTINUE
      IF(DEBUG)WRITE(JOSTND,9002) I,X,CRNEW(I),ICR(I),ICRFST
 9002 FORMAT('0 IN CROWN 9002 WRITE I,X,CRNEW,ICR,ICRFST = ',I5,2F10.5,
     &2I5)
...
C----------
      IF(ICRFST.EQ.0 .OR. ICR(I).EQ.0) GO TO 9052
 ...
 9052 ICRI = CRNEW(I)+0.5
      IF(ICRFST.EQ.0 .OR. ICR(I).EQ.0)THEN
...
C----------
      IF(ICRFST.EQ.0 .OR. ICR(I).EQ.0)GO TO 55
      CRLN=HT(I)*ICR(I)/100.
C----------
...
C----------
     ...
     & CHG
 9004 FORMAT('0 CRMAX=',F10.2,' CRLN=',F10.2,
...
C----------
   55 IF (ICRFST .NE. 0 .OR. ITRUNC(I).EQ.0) GO TO 59
      HN=NORMHT(I)/100.0
     ...
     & ICRI,CL
 9030 FORMAT('0  IN CROWN 9030 I,ITRUNC,NORMHT,HN,HD,ICRI,CL = ',
     & 3I5,2F10.3,I5,F10.3)
      GO TO 59
C----------
...
C----------
   59 CONTINUE
      ...
      ICR(I)= ICRI
   60 CONTINUE
      IF(ICRFST.EQ.0 .AND. ICFLG(ISPC).EQ.1)THEN
        CRNMLT(ISPC)=1.0
        ICFLG(ISPC)=0
      ENDIF
   70 CONTINUE
...
C
 ...
 9010 FORMAT('0 LEAVING CROWN 9010 FORMAT ITRN,ICR= ',I10,/,
     & 43(1H ,32I4,/))
      ICRFST = 1
