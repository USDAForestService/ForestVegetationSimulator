      SUBROUTINE DBSFMBMCMP(IYR)
      IMPLICIT NONE
C
C $Id
C
C     PURPOSE: TO POPULATE A DATABASE WITH THE TREE BIOMASS
...
C
      ...
      IF(IRCODE.EQ.1) THEN
        IF(TRIM(DBMSOUT).EQ."ACCESS") THEN
          SQLStmtStr='CREATE TABLE FVS_Biocmp('//
...
C
        ...
        ENDIF
C
        IF(ISPOUT37.EQ.1)CSPECIES=ADJUSTL(TRIM(JSP(ISP(I))))
        IF(ISPOUT37.EQ.2)CSPECIES=ADJUSTL(TRIM(FIAJSP(ISP(I))))
        IF(ISPOUT37.EQ.3)CSPECIES=ADJUSTL(TRIM(PLNJSP(ISP(I))))
C
        IT=I
C
...
C
     ...
     -         int(len_trim(SQLStmtStr),SQLINTEGER_KIND))
C
         IF (iRet.NE.SQL_SUCCESS) IFMBMCMP=0
         CALL DBSDIAGS(SQL_HANDLE_STMT,StmtHndlOut,
