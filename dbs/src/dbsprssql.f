      ...
      IMPLICIT NONE
C
C  **DBSPRSSQL-- DATE OF LAST REVISION:  10/31/2011
C
C
...
C
      ...
      CHARACTER*20 CTOK,NUMSTR,CMPUSTR
      CHARACTER*7 VVER
      INTEGER KODE,I,IRETVAL
...
C               CHECK AGAINST COMPUTES AND OTHER EVENT MONITOR VARS
                CTOK = TOKEN
                CALL ALGKEY(CTOK,LEN_TRIM(CTOK),IRETVAL)
                IF(IRETVAL.EQ.0) THEN
C                 FOUND THE TOKEN
                  CALL EVLDX(RVAL,1,IRETVAL)
                ENDIF
                IF(IRETVAL.EQ.0) THEN
