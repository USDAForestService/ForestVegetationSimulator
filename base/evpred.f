...
C----------  
      IF(VARIANT.EQ.'NC' .AND. IFOR.NE.4 .AND. IFOR.NE.5)THEN
        GO TO 20
      ELSEIF((VARIANT.EQ.'CA'.OR.VARIANT.EQ.'OC').AND.IFOR.LT.6)THEN
        GO TO 20
      ELSEIF(VARIANT.EQ.'SO' .AND. (IFOR.GT.3 .AND. IFOR.LT.10))THEN
        GO TO 20
      ELSEIF(VARIANT.EQ.'WS')THEN
        GO TO 20
      ELSE
        GO TO 500
      ENDIF
...
C
      ...
      ENDIF
      IF(VARIANT.EQ.'WS' .AND. (ISPC.EQ.7 .OR. ISPC.EQ.11))THEN
        ADHW=ADHW + D*P
        SUMTPA=SUMTPA + P
      ELSEIF(VARIANT.EQ.'NC' .AND. (ISPC.EQ.5 .OR. ISPC.EQ.7 .OR.
     &       ISPC.EQ.8 .OR. ISPC.EQ.11))THEN
        ADHW=ADHW + D*P
        SUMTPA=SUMTPA + P
      ELSEIF((VARIANT.EQ.'CA' .OR. VARIANT.EQ.'OC').AND.ISPC.GE.26)THEN
        ADHW=ADHW + D*P
        SUMTPA=SUMTPA + P
      ELSEIF(VARIANT.EQ.'SO' .AND. ((ISPC.GE.21 .AND. ISPC.LE.31) .OR.
     &       ISPC.EQ.33))THEN
        ADHW=ADHW + D*P
        SUMTPA=SUMTPA + P
      ENDIF
