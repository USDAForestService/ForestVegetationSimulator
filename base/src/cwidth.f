...
C     IRTN -- HAS A VALUE OF 0 IF FVS IS COMPUTING THE CROWN WIDTHS, 1 IF 
C             CROWN WIDTHS ARE COMPUTED USING ORGANON EQUATIONS.
C----------
      LOGICAL DEBUG,LINCL
      REAL PRM(5)
      INTEGER MYACTS(1)
...
C----------
      ...
      CRWDTH(I)=0.0
C----------
C  SET PARAMETERS FOR CALL TO **ORGMCW**. THIS EXISTS AS AN ENTRY
C  POINT IN **EXORGANON** FOR ALL VARIANTS EXCEPT OC AND OP. IT
C  IS A SUBROUTINE IN SUBDIRECTORY ORGANON\SRC FOR OC AND OP.
C----------
      IRTN=0
      CW=0.
C----------
C  IF THE USER ENTERED CROWN WIDTH EQUATIONS FOR THIS SPECIES,
C  USE THOSE.
C----------
      IF(LSPCWE(ISPC))THEN
     ...
     &       CWDL2(ISPC)*(DBH(I)**CWDL3(ISPC))
        ENDIF
      ELSE
        CALL ORGMCW(ISPC,D,H,JOSTND,IMODTY,IRTN,CW,DEBUG)
        IF(IRTN .EQ. 0) THEN
          IWHO=0
      ...
      ENDIF
      CRWDTH(I)=CW
      IF(DEBUG)WRITE(JOSTND,*)' LIVE: I,ISPC,D,CR,CW,IRTN= ',
     &I,ISPC,D,CR,CW,IRTN
   50 CONTINUE
C
  100 CONTINUE
