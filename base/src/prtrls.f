      ...
      IMPLICIT NONE
C----------
C  **PRTRLS--BASE   DATE OF LAST REVISION: 01/22/2012
C----------
C
...
C
      ...
      ENDIF
C
C     MAKE SURE THE OUTPUT FILE IS OPENNED, IF NOT OPEN ONE USING THE 
C     KEYWORD FILE NAME PREFIX.
C
      INQUIRE(UNIT=KOLIST,opened=LCONN)
      IF (.NOT.LCONN) THEN
        CALL getkeywrd(RECORD,len(RECORD))
        IF (RECORD.NE.' ') THEN
          I = index(RECORD,".k")
          IF (I == 0) I=index(RECORD,".K")
          IF (I == 0) I=len_trim(RECORD)
          RECORD=TRIM(RECORD(:I-1))//".tls"
          OPEN(UNIT=KOLIST,FILE=TRIM(RECORD),STATUS='replace')
        ENDIF 
      ENDIF                    
C
