...
14510   FORMAT (/A8,'   DATE/CYCLE=',I5,'; OVERLAP COEFICCIENT=',F8.6)
      ...
      GOTO 10
C
C  ==========  OPTION NUMBER 146: PERMFRST  ==========================PERMFRST
C  THIS OPTION IS ONLY VALID FOR THE ALASKA (AK) VARIANT.
C
14600 CONTINUE
      IF (VARACD .NE. 'AK') THEN
        WRITE(JOSTND,14614) KEYWRD
        GOTO 10
      ENDIF
      IDT=0
C
...
C
     ...
     >                   IPRMPT,IRECNT,ICYC)
      ...
      ENDIF
C
C     IF THE DATE FIELD (1) IS ZERO OR BLANK JUST REPORT STATUS OF
C     PERMAFROST MODIFIER AND DO NOT SCHEDULE ACTIVITY.
C
      IF (LNOTBK(1)) IDT=IFIX(ARRAY(1))
      IF (IDT .EQ. 0) THEN
        IF (ARRAY(2) .EQ. 0.0) THEN
          WRITE(JOSTND,14612) KEYWRD,IDT
        ELSE
          LPERM = .TRUE. 
          WRITE(JOSTND,14613) KEYWRD,IDT
        ENDIF
        GOTO 10
      ENDIF
      ...
      IF(LKECHO) THEN
        IF (ARRAY(2) .EQ. 0.0) THEN
          WRITE(JOSTND,14610) KEYWRD,IDT
        ELSE
          WRITE(JOSTND,14611) KEYWRD,IDT
        ENDIF
      ENDIF
14610 FORMAT (/A8,'   DATE/CYCLE=',I5,'; PERMAFROST MODIFIER INACTIVE.')
14611 FORMAT (/A8,'   DATE/CYCLE=',I5,'; PERMAFROST MODIFIER ACTIVE.')
14612 FORMAT (/A8,'   DATE/CYCLE=',I5,'; PERMAFROST MODIFIER IS ',
     >       'INACTIVE. ACTIVITY WILL NOT BE ACHEDULED.')
14613 FORMAT (/A8,'   DATE/CYCLE=',I5,'; PERMAFROST MODIFIER IS ',
     >       'ACTIVE. ACTIVITY WILL NOT BE ACHEDULED.')
14614 FORMAT (/A8,'   PERMAFROST MODIFIER VALID FOR ALASKA (AK) ',
     >        'VARIANT ONLY, KEYWORD IGNORED.')
      GOTO 10
C
      END