...
C  IF USER SETS THIS TO UNEVEN-AGED BUT DOES NOT SET STAND AGE USING
C  THE STDINFO KEYWORD, THEN THIS WILL GET SET BACK TO EVEN-AGED
C  IN **SITSET**
C
      IF (LNOTBK(2)) THEN
        INDS(4) = IFIX(ARRAY(2))
...
C
      ...
      LNOTRE=.TRUE.
C
C  IF STARTING FROM A PREPPED FILE, TURN THE THREE CALIBRATION VARIABLES
C  TO 0=NO FOR CALL TO PREPARE IN CRATET.
C  MAKE SURE THE PREP FLAG IS STILL SET TO FALSE
C
      INDS(1) = 0
      INDS(2) = 0
      INDS(3) = 0
