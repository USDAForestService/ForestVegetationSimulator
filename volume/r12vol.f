...
C 2018/10/10 YW ADDED VALID SPECIES CHECK
...
C--   IF DBHOB OR HT1PRD EQUALS ZERO THEN DON'T CALCULATE THE VOLUME
      ...
      IF(HT1PRD .LE. 0.0 ) THEN
         ...
         GO TO 1000
      ENDIF
C--   CHECK VALID SPECIES (2018/10/10 YW)
...
C--   SUBROUTINE "R12TAP" IS THE PROFILE MODEL
      IF(CUTFLG.EQ.1) THEN
        HT2 = HT1PRD
        VTYPE = 1
C     TOTAL CUBIC VOLUME SHOULD USE HTTOT BASED ON SHARPNECK MODEL(YW 2018/11/07)   
        TOPD = 0.0
        IF(HTTOT.LT.0.1)THEN
          HTTOT = HT1PRD
          TOPD = MTOPP
        ENDIF
C        CALL R12TAP(EQNUM,DBHOB,HT1PRD,HT2,MTOPP,FCLASS,VTYPE,TCVOL)
