      ...
      IMPLICIT NONE
C----------
C  **PPIN--PPBASE   DATE OF LAST REVISION:  07/31/08
C----------
C
...
C                        OPTION NUMBER 32 MXCLRCUT
      IF (LNOTBK(1)) HVMXCC=ARRAY(1)
      LHVMXC=.TRUE.
      WRITE (JOPPRT,4210) KEYWRD,HVMXCC
 4210 FORMAT (/1X,A8,'   MULTISTAND POLICY SELECTION WILL LIMIT ',
     >  'CONTIGUOUS CLEAR CUTS TO ',F6.2,' ACRES FOR SELECTED STANDS.'/
...
C                        OPTION NUMBER 37 NBDENEFF
      LNEDNG=.TRUE.
      IF (LNOTBK(1)) HXSIZE=ARRAY(1)
      WRITE (JOPPRT,4710) KEYWRD,HXSIZE
 4710 FORMAT (/1X,A8,'   THE GROWTH OF EACH STAND IS PARTILY ',
...
C                        OPTION NUMBER 41 DEFCCUT
      IF (LNOTBK(1)) DCTOP=ARRAY(1)
      IF (LNOTBK(2)) DCTPA=ARRAY(2)
      WRITE (JOPPRT,5110) KEYWRD,DCTOP,DCTPA
 5110 FORMAT (/1X,A8,'   A STAND IS CONSIDERED CLEAR CUT WHEN ',
     >        'THE TOP HEIGHT IS LESS THAN ',F6.1,' AND '/T13,
     >        'THE DENSTIY IS IS LESS THAN ',F6.1,' TREES PER ACRE')
      GOTO 10
 5200 CONTINUE
