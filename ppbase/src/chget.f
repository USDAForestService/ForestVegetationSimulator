      ...
      IMPLICIT NONE
C----------
C  **CHGET--PPBASE   DATE OF LAST REVISION:  10/15/12
C----------
C
...
C
    ...
    5 CONTINUE
      DO 10 J=1,26
      CALL CHREAD(CBUFF,IPNT,LNCBUF,NPLT(J:J),K)
   10 CONTINUE
      DO J=1,LEN(DBCN)
        CALL CHREAD(CBUFF,IPNT,LNCBUF,DBCN(J:J),K)
      ENDDO
C
      IF (LBSETS) THEN
         IF (LENSLS.GT.0) THEN
