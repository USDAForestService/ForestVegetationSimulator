      ...
      IMPLICIT NONE
C----------
C  **PUTSTD--PPBASE   DATE OF LAST REVISION: 07/21/11
C----------
C
C     WRITES A STAND TO THE ALL DATA NUMERIC FILE, AND THE ALL
C     DATA CHARACTER FILE, FROM MEMORY.
C
...
C     OF LENGTH 2.
C
      INTEGER MXR,MXI,ILIMIT,IPNT,K,I,II,MXL,INTS(MXI)
      LOGICAL LOGICS(MXL),LCVGO,LMORED,LRR1,LRR2,LFM,LBWE
      PARAMETER (MXR=128,MXL=36,MXI=114)
...
C
C     SET THE BUFFER SIZE LIMIT.  IT IS EITHER THE RECORD LENGTH,
C     OR THE LENGTH OF AVAILABLE CONTIGUOUS SPACE IN THE BUFFER,
C     WHICHEVER IS LESS.
C
      ILIMIT=IRECLN
      IF (ILIMIT.GT.MAXBUF) ILIMIT=MAXBUF
C
C     STORE THE DIRECT ACCESS DATA RECORD IN THE STAND TABLES FOR
C     LATER USE.  NOTE THAT IWTREC IS FIRST SET IN SPAMGM AND
C     UPDATED IN STASH AND CHSTSH AS NEEDED.
C
      ISNKEY(ISTND,2)=IWTREC
      IF (PDEBUG) THEN
         WRITE (JOPPRT,'(/'' IN PUTSTD: ISTND,IWTREC='',2I7)')
     >          ISTND,IWTREC
         IF (LBSETS.AND.LENSLS.GT.0) WRITE (JOPPRT,'('' IN PUTSTD: '',
     >      ''SLSET='',A)') SLSET(1:LENSLS)
      ENDIF
C
...
C
   ...
   22 CONTINUE
      ...
      CALL BFWRIT (WK3,IPNT,ILIMIT,TSTV1,  46,        2)
...
C
      CALL CHPUT
C
C     WRITE THE GENDEFOL/BUDWORM MODEL CHARACTER VARIABLES, IF ACTIVE.
C
      CALL BWEPPATV (LBWE)
      IF (LBWE) CALL BWEPPPT (WK3,IPNT,ILIMIT,2)
C
C     TABLE THE LAST WRITE RECORD.
C
      ISNKEY(ISTND,3)=IOSTRC
C
      RETURN
      END
