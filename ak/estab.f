...
C TYPE. 
C
C THE FOLLOWING FOREST TYPES ASSUME THE MEDIAN TPP VALUE FOR
C INGROWTH PREDICTIONS:
C-----------
C BLACK SPRUCE
C MOUNTAIN HEMLOCK
C YELLOW CEDAR
...
C RED ALDER
C
C THE FOLLOWING FOREST TYPES ASSUME THE MEAN TPP VALUE FOR
C INGROWTH PREDICTIONS:
C-----------
C WHITE SPRUCE
C QUAKING ASPEN
...
C-----------
     ...
     &  MAXING/11,14,4,10,10,8,10,3,4,9,9,11,2,0/
      DATA HABTYP/
     &  'PSME/ VAGL, LIBO, VACA',
...
C     ESADVH CALCULATES HEIGHT OF BEST TREE (HHT)
 ...
 7007 FORMAT('DELAY TO GERM=',F10.4,'  SPECIES=',I5,
     ...
     &' HTT=',HHT,' TRAGE=',TRAGE,' GENTIM=',GENTIM,' STOMLT=',
     & STOMLT(I),' DILATE=',DILATE
   95 CONTINUE
...
C     BEST TREES NOT SUBJECT TO MORTALITY FOR THE FIRST 20 YEARS.
      ...
      IF(DEBUG)WRITE(JOSTND,*)'IN DO LOOP 33 CHECKING VALUES.',
     & ' N=',N,' ITRN=',ITRN,' ISP=',ISP(ITRN),' ITRE=',ITRE(ITRN),
     & ' PROB=',PROB(ITRN),' ABIRTH=',ABIRTH(N),' IESTAT=',IESTAT(ITRN),
...
C     CALL BLISTER RUST TO PROCESS PLANTED TREES.
      ...
      IF(DEBUG)WRITE(JOSTND,*)'END OF LOOP 226 CHECKING VALUES',
     & ' ITRN=',ITRN,' II=',II,' ISP=',ISP(ITRN),' ITRE=',ITRE(ITRN),
     & ' PROB=',PROB(ITRN),' FTEMP2=',FTEMP2,' EXPAND=',EXPAND,
     & ' XCSMAX=',XCSMAX,' DUPNPT=',DUPNPT,' ABIRTH=',ABIRTH(ITRN),
     & ' IDTREE=',IDTREE(ITRN),' IDCMP1=',IDCMP1,' ITMP=',ITMP
  226 CONTINUE
  228 CONTINUE
