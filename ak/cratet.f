...
C----------
C  TEMESGEN MODEL FORM--NO CALIBRATION
  110 CONTINUE
      ...
      ENDIF
        ...
        DX = HTT14(ISPC)
        EX = HTT15(ISPC)
        H=4.5+(AX+BX*PBAL+CX*PBA)*(1-EXP(DX*D**EX))
        IF (DEBUG) WRITE(JOSTND,88) ISPC,AX,BX,CX,DX,EX,D,PBAL,PBA,H   
  111 CONTINUE    
      IF(TKILL) GO TO 112
      HT(II)=H
      K4=K4+1
  ...
  113 CONTINUE
      GO TO 130            
C  WYKOFF MODEL FORM--CALIBRATION                  
  116 CONTINUE 
      IF(K2.EQ.0) GO TO 140     
      DO 130 JJ=1,K2
      II=IND2(JJ)
