      SUBROUTINE DUBSCR(ISPC,D,H,RDEN,QMDPLT,CR)
      IMPLICIT NONE
C----------
C AK $Id$
C----------
...
C
      INTEGER ISPC
C
      REAL BACHLO,CR,D,FCR,H,RDANUW,SD,RDEN,QMDPLT
C
      REAL CRSD(MAXSP)
...
C    STANDARD ERROR OF REGRESSIONS FOR EACH SPECIES
      DATA CRSD/
     &  0.1755, 0.1755, 0.1630, 0.1647, 0.1536,
     &  0.1536, 0.1894, 0.1755, 0.1583, 0.2070,
     &  0.1751, 0.1643, 0.1536, 0.1532, 0.1532,
     &  0.1439, 0.1439, 0.1247, 0.1354, 0.1574,
     &  0.1818, 0.1818, 0.1574/
C
C-----------
C  CHECK FOR DEBUG.
C-----------
...
C
C     DO SOME DEBUG
      IF(DEBUG)WRITE(JOSTND,*)'IN DUBSCR -ISPC,D,H,RDEN,QMDPLT,FCR,CR ',
     & ISPC,D,H,RDEN,QMDPLT,FCR,CR
C
