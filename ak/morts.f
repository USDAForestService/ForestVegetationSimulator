...
C     SURVIVAL INTERCEPT COEFFICIENTS
     ...
     & 5.159052, 5.441808, 4.585373, 2.987803, 2.987803,
...
C     SURVIVAL DBH^2 COEFFICIENTS
     ...
     & -0.00105,  -0.000952, -0.020537, -0.014038, -0.014038,
...
C     NOT USED (ZERO VALUE) FOR SF, AF, YC, SS, LP, RC, WH, MH
      DATA BM4/
     & -0.002328, -0.002328, -0.001162, -0.004365, -0.004414,
     & -0.004414, -0.004365, -0.002328, -0.00111,  -0.003401,
     & -0.000402, -0.000794, -0.004414, -0.005462, -0.005462,
...
C     MINIMUM DIAMETER USED FOR SURVIVAL CALCULATIONS
      DATA DSURV/
     & 0.75, 0.75, 0.50, 0.10, 0.10, 0.10, 0.10, 0.50, 0.75, 0.50,
...
C CALCULATE SURVIVAL
     ...
     &  BM5(ISPC)*PTBAL/DTEMP

      RIP = EXP(RIP)/(1 + EXP(RIP))

C DETERMINE IF DEBUG IS NEEDED
      IF (DEBUG)WRITE(JOSTND,*)'IN MORTS EQNS I,ISPC,RIP,D,DTEMP,
     & PTBAL=', I,ISPC,RIP,D,DTEMP,PTBAL
C----------
