      ...
      IMPLICIT NONE
C----------
C  **ESUCKR--NI23   DATE OF LAST REVISION:   06/20/11
C
C  CURRENT NI SPROUTING SPECIES:
C  18  AS  QUAKING ASPEN  FROM UT VARIANT
C  19  CO  COTTONWOOD  FROM CR VARIANT
C  20  MM  MOUNTAIN MAPLE  FROM UT VARIANT (GROWN AS ASPEN)
C  21  PB  PAPER BIRCH  FROM UT VARIANT (GROWN AS ASPEN)
C  22  OTHER HARDWOODS  FROM CR VARIANT (GROWN AS COTTONWOOD)
C----------
COMMONS
C
      INCLUDE 'PRGPRM.F77'
...
C----------
C  PACIFIC YEW (17) SPROUT COUNT IS CUT IN HALF
C----------
      IF(ISSP.EQ.17)THEN
        NUMSPR=1
      ELSE
...
C
      ...
      IF(ISSP.GE.18 .AND. ISSP.LE.22)THEN
